<x-guest-layout>
  <div class="relative min-h-screen py-12 bg-bg">
    ...
    <div class="max-w-2xl p-8 mx-auto bg-white border border-gray-100 shadow-lg rounded-2xl">
      <!-- Heading -->
      <h1 class="mb-6 text-3xl font-bold text-accent">Edit Book</h1>
      ...
      <form action="{{ route('books.update', $book->id) }}" method="POST" class="space-y-6">
        ...
        <div>
          ...
          <input type="text" name="title" id="title" value="{{ old('title', $book->title) }}"
            class="w-full border-gray-300 rounded-lg focus:ring-2 focus:ring-accent focus:border-accent"
        ...
        <div class="flex items-center justify-end gap-3">
          <a href="{{ route('books.show', $book->id) }}"
            class="px-4 py-2 bg-gray-200 rounded-lg transition-3s text-text hover:bg-gray-300">
            Cancel
          </a>
          <button type="submit"
            class="px-5 py-2 font-medium text-white rounded-lg shadow transition-3s bg-accent hover:bg-accent-hover">
            Update Book
          </button>
        </div>
      </form>
